...
const time_period_menuElement = document.getElementById('time_period_menu')

const tariff_menuElement = document.getElementById('tariff_menu')

const priceAvailable_menuElement = document.getElementById('priceAvailable_menu')


const totalConsumptionInput = document.getElementById('totalConsumptionInput')

const peakConsumptionInput = document.getElementById('peakConsumptionInput')

const half_peakConsumptionInput = document.getElementById('half_peakConsumptionInput')

const off_peakConsumptionInput = document.getElementById('off_peakConsumptionInput')
...
const pricing_box_container = document.querySelector('.pricing_box_container')
...
let selectedCountry, selectedState,
  selectedTimePeriod, isPriceAvailable, contracted_power, duration, total__consumption_per_month, peak_hour_consumption, half_peak_hour_consumption,
...
let FROM_SUPABASE, FILTERED_DATA, TF_VALUE = Math.random().toFixed(4),
  ...
  CONTAGEM, selectedContractedPowerCode;
...
contracted__power_menuElement.addEventListener('change', (e) => getContractedPowerInfoFromSupabase(e.target.value))


priceAvailable_menuElement.addEventListener('change', showPricingFields)


time_period_menuElement.addEventListener('change', showConsumptionFields)
...
function showPricingFields(e) {
  (e.target.value === "true") ?
  pricing_box_container.classList.remove('hide'):
    pricing_box_container.classList.add('hide')
}

function showConsumptionFields(event) {
  if (event.target.value === "1") {
    totalConsumptionInput.classList.remove('hide')

    peakConsumptionInput.classList.add('hide')
    half_peakConsumptionInput.classList.add('hide')
    off_peakConsumptionInput.classList.add('hide')
  } else if (event.target.value === "2") {
    peakConsumptionInput.classList.remove('hide')
    off_peakConsumptionInput.classList.remove('hide')
    half_peakConsumptionInput.classList.add('hide')
    totalConsumptionInput.classList.add('hide')
  }
  else if (event.target.value === "3") {
    peakConsumptionInput.classList.remove('hide')
    off_peakConsumptionInput.classList.remove('hide')
    half_peakConsumptionInput.classList.remove('hide')
    totalConsumptionInput.classList.add('hide')
  }
}
...
function calculateGrandTotal(variable_price) {
  ...
  let totalWithoutVAT = parseFloat((FIXED_PRICE * total_number_of_days) + (price_per_contracted_power * parseFloat(variable_price) * total_number_of_days) + (0.07 * duration) + (0.001 * total__consumption_per_month) + (0.093634 * total_number_of_days)).toFixed(4)
...
function evaluateVAT(totalWithoutVAT) {
  ...
  if (selectedState == "Madeira" || selectedState == "Azores") {
    ...
    if ((contracted_power <= 6.9) && (total__consumption_per_month < 150)) {
      ...
      expectedVAT = parseFloat((totalWithoutVAT * 23) / 100)
    }
  }


  let grandTotal = parseFloat(totalWithoutVAT + expectedVAT).toFixed(2)


  console.log("type of expectedVAT " + typeof expectedVAT, expectedVAT)



  grandTotalBillElement.innerText += ("\n Bill with VAT will be " + grandTotal)
...
// making api call to the database to retrieve necessary information
...
async function getContractedPowerInfoFromSupabase(selectedContractedPower) {
  const _supabase = supabase.createClient(SUPABASE_URL, CLIENT_API_KEY);
  ...
  const { data, error } = await _supabase
    .from(tableName)
    .select('').eq('Pot_Cont', replaceCommaWithDot(selectedContractedPower))
    .neq('TF', null)
    .limit(1);
  ...
  // Otherwise use this data to initialize some variable(s)
  /*TF_VALUE = replaceCommaWithDot(data[0]?.TF)

  TV_TVFV_TVP = replaceCommaWithDot(data[0])

  TVV_TVC = replaceCommaWithDot(data[0]?.TVV | TVC)

  TVVz = replaceCommaWithDot(data[0]?.TVVz)
*/
  CONTAGEM = data[0]?.Contagem

  selectedContractedPowerCode = data[0]?.COD_Proposta
  if (selectedContractedPowerCode) {
    getOfferByContractedPowerCode(selectedContractedPowerCode)
  }
}

async function getOfferByContractedPowerCode(selectedContractedPowerCode) {
  console.log(selectedContractedPowerCode)
  const _supabase = supabase.createClient(SUPABASE_URL, CLIENT_API_KEY);

  // Specify the name of the table from which you want to fetch data
  const tableName = 'Tariff_Info';

  // Fetch the data from Supabase
  const { data, error } = await _supabase
    .from(tableName)
    .select('').eq('Fornecimento', "ELE").eq('COD_Proposta', selectedContractedPowerCode)
...
/* Utility function */
function replaceCommaWithDot(givenText) {
  if (givenText.includes('.'))
    return givenText.replace('.', ',')

  /*if (givenText.includes('.'))
    givenText.toString().replace('.', ',')
*/

}

function convertToString(pipedText) {
  return pipedText.toString()
}

function showOffers(offers) {
  
    resultElement.innerHTML += offers[0]
